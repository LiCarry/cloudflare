...
function classify(req) {
  ...
  // Some PaaS edges (e.g. Railway) normalise X-Forwarded-For to the original
  // visitor IP, so the Cloudflare edge IP never appears there. For those
  // platforms, pair this with a Cloudflare Transform Rule that force-sets a
  // secret header on every proxied request (attacker cannot know the value).
  const secret = process.env.CF_SHARED_SECRET;
  const secretOk = Boolean(secret) && req.headers["x-cf-shared-secret"] === secret;
  return {
    peerIp: peer.ip,
    peerSource: peer.via,
    // `cf-ray` only exists on requests that traversed the Cloudflare edge.
    traversedCloudflare: Boolean(req.headers["cf-ray"]),
    allowed: fromLoopback || fromCloudflare || secretOk,
    reason: fromLoopback
      ? "loopback (Cloudflare Tunnel)"
      : fromCloudflare
        ? "Cloudflare IP range"
        : secretOk
          ? "shared secret header (set by Cloudflare Transform Rule)"
          : "not a Cloudflare IP",
  };
}
